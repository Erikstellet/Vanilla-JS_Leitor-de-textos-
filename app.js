...
const buttonReadText = document.getElementById('read');
...
[
    ...
    { img: './img/outside.jpg', text: 'Quero sair'           },
...
{ 
    const selectedVoice = voices.find(voice => voice.name === target.value);
...
main.addEventListener('click', event  => 
{
    ...
    const conditionalTag = ['IMG','P'].some(tag =>
    {
       return clickedElement.tagName.toLowerCase() === tag.toLowerCase()
    });
...
{ 
    ...
    {
       acc += `<option value="${ name }"> ${ name } | ${ lang } </option>`
       return acc;
    }, '')
};
